...
int main() {
	...
	long double length_needed = 0;
	for (int i = 1; i < c_hull.size(); i++) {
		length_needed += sqrt((c_hull[i].x - c_hull[i - 1].x) * (c_hull[i].x - c_hull[i - 1].x)
			+ (c_hull[i].y - c_hull[i-1].y) * (c_hull[i].y - c_hull[i - 1].y));
	}
	cout << length_needed << '\n';
}
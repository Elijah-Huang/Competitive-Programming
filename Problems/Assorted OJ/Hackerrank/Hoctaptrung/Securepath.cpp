...
int main() 
{
	...
	// lowerbound is this and it is achieved by pairing leaves across a root and 
	// pairing the final leaf with the root if the number of leaves is odd
	// the optimal location to pair them over is the "centroid" where
	// each subtree (including the one "above") has <= tot_leaves/2
	cout << (leaves + 1) / 2; 
}